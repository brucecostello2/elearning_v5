...
| **Status** | IN PROGRESS. Phases 2a/2c/2d done and 2e (deploy script) done; 2b/2f/2g/2h remain (see carry-forward). Node-IP model: node-01 `.env` holds the full registry as literal `NODE_01_IP..NODE_06_IP` (the earlier `SUBNET_PREFIX` form was dropped per operator — admin assigns IPs at the router/machine and registers them here); `node01.yml` `x-gpu-service-urls` anchor composes the 12 GPU URLs from them. Star topology: each GPU node references only node-01 via a single `NODE_01_IP` pointer. node02-06 compose now parse (a fatal duplicate-`<<:` bug was fixed) and resolve node-01 refs to `${NODE_01_IP}`; all `.env*.template` carry the model; `deploy-node.sh` repaired. Commits `fa6f4db` (2a), `49b736d` (node-01 templates), `7c374e5` (node02-06 compose+templates + parse fix), `bfcec00` (deploy-node.sh). node-01 containers healthy; node02-06 validated via `docker compose config` only (no GPU hardware). |
...
| **Carry-forward action** | Remaining, each behind a verify gate: (2b) worker/API config unification — reconcile `shared/config.py` (unprefixed) vs `ivgs-workers/config.py` (IVGS_-prefixed) onto one authority, remove hardcoded `10.10.0.x`/hostname fallback defaults (fail-fast), rebuild `ivgs-api` + `ivgs-workers`, coordinated deploy (the only phase with live-startup risk); (2e remainder) `scripts/restore.sh` (lines 250/497 still hold `10.10.0.x`); (2f) `.github/workflows/cd-deploy.yml`; (2g) docs (runbook/README/audit/pre-deploy) + SeaweedFS bind configs (investigate); (2h) `tests/spec_compliance/test_no_hardcoded_ips.py` (subsumes P2.14) + widen pre-commit hook. OPEN CAVEATS: (i) GPU stacks + `deploy-node.sh` are fixed by inspection but have NEVER run end-to-end (no GPU hardware) — full shakeout required when hardware lands (relates to P3 Phase H); (ii) `deploy-node.sh` migration references `POSTGRES_PASSWORD` not sourced into the script shell — resolve in that shakeout. |
