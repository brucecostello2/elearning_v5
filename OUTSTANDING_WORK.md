...
| id | Fact |
|---|---|
| **RC-I4** | ✅ **CLOSED 2026-08-29 (WP-IVGS-12). CAUSE: A NIGHTLY OPERATOR POWER-DOWN**, supplied by the operator and recorded here as the cause of **both** coordinated-boot events. The row previously said *"I did not read the journal deeply enough to name the cause"* and floated an unattended-upgrades theory off `/var/log/apt/history.log`; **that theory is withdrawn.** Event 1, 2026-08-28: .94 02:31:48, .91 02:32:41, .93 02:34:31, .92 03:16:11. **Event 2, 2026-08-29, measured by WP-IVGS-12 from `ivgs-celery-default` heartbeat supervision**: node-02 last heartbeat **05:38:58.901881+00**, node-04 ~05:39:15, node-03 ~05:39:31 — **all inside 33 seconds**, with .94 and .95 gone in the same window, `arp INCOMPLETE` on all five and `connect: No route to host` from node-01 while the gateway, .7, .51 and .96 all answered. Scheduler `/fleet` read `alive_nodes: 0`. Restored by the operator ~11:20 UTC; `ivgs-vllm-primary` came back on the pinned digest `sha256:3dbe092e…` unchanged. ⚠ **Operational consequence, not a defect: any package whose acceptance needs the GPU fleet must not assume overnight availability.** ✅ node-04's 450 W cap HELD (`power.limit = 450.00 W`, measured post-reboot, event 1) |
...
## RC-Q — WP-IVGS-12: the Design Core (2026-08-29)

Phase 1 of `dev/design/IVGS_Root_Cause_Audit_and_Recovery_Plan_2026-08-29.md`,
against `dev/design/Instructional_Design_Foundation_for_IVGS_2026-08-29.md`.
Report: `dev/workpackages/reports/WP-IVGS-12-DESIGN-CORE-report_2026-08-29.md`.

| row | statement |
|---|---|
| ⛔ **RC-Q1** | **`guided_json` IS ACCEPTED WITH HTTP 200 AND SILENTLY IGNORED, AND SO IS EVERY UNKNOWN TOP-LEVEL BODY MEMBER.** Measured 2026-08-29 against the pinned engine `vllm/vllm-openai@sha256:3dbe092e…` (vLLM `0.19.2rc1.dev134+gfe9c3d6c5`) on node-02. Output byte-identical to an unconstrained call; **still HTTP 200 when handed `{"type":"not_a_json_type"}`, when handed the bare integer `12345`, for `guided_choice`, and for a field name invented for the test.** ⛔ **THIS IS A TRAP FOR EVERY FUTURE STRUCTURED-OUTPUT CALL ON THIS FLEET, NOT ONLY FOR THE DESIGN CONTRACT** — any request-body knob that this engine does not recognise is discarded without comment, so a feature built on one reports success forever and does nothing. The recovery plan §1 RC-A step 4 and §4 Phase 1 both prescribed `guided_json`; both are bannered. ✅ **MECHANISM OF RECORD, ruled by the operator 2026-08-29: `response_format: {"type": "json_schema", "strict": true}`**, measured ENFORCING on a 7.9 kB nested contract (closed enums, `oneOf`, `minItems`, `additionalProperties:false`). `structured_outputs: {"json": …}` measured equivalent, held in reserve. `design_core/contract.response_format_for` REFUSES the legacy names by name with the measurement in the message. Probes banked at `dev/workpackages/reference/wpivgs12-prompt-stack/` |
| ⛔ **RC-Q2** | **THE UPLOADED SCRIPT OF EVERY PRE-0046 PROJECT IS UNRECOVERABLE FROM THE DATABASE. STATED PLAINLY SO NOBODY HUNTS FOR IT.** One path creates a `transcripts` row (`ivgs-api/app/services/transcript_service.py:157`) and writes the extracted upload into **`refined_text`**; stage 1 reads that same column (`ivgs-workers/tasks/stage1_transcript.py:208`) and PATCHes its paraphrase back over it (`:241`). There has never been an `original_text` column — the worker's `TranscriptRecord.original_text` is populated FROM `refined_text`. Measured 2026-08-29, one 3,172-byte upload across three of the operator's projects: `4ca0d5c5` **1,866** chars, `9c29b1d1` **1,851**, `c12fa967` **1,615** — three different paraphrases, no original. ⛳ **This is why recovery-plan §3 item 4 happened**: "nobody compared output narration to input script" — the comparison was not possible. A re-run refines a paraphrase of a paraphrase. **Migration 0046 adds `transcripts.source_text`, written once by the upload path and by nothing else, and DOES NOT BACKFILL** (operator ruling 2026-08-29): copying today's `refined_text` in would enshrine the paraphrase AS the original and destroy the only evidence the two differ. ⓘ **The bytes may still exist OUTSIDE the database**, in SeaweedFS behind `transcripts.original_asset_id`, for rows whose asset was not cleared by `ON DELETE SET NULL`. Recovery from there is an opt-in script and is NOT a migration. **Do not look for a database column; there is none and there never was.** |
| ⛔ **RC-Q3** | **A 64-CHARACTER CHAT REFUSAL WAS RECORDED AS A REFINED TRANSCRIPT, AND NOTHING COULD HAVE CAUGHT IT.** Project `0361c667-272b-4f66-9a08-de791f1f92a6`: `transcripts.refined_text` is *"Sure! Please provide the raw transcript you'd like me to refine."* — **64 characters written over a 540-byte upload**, job green. Joined to the WP-00 swallowed-failure register as **instance 18**. **The gap is that no check anywhere asks whether the output is a transcript at all**: `_refine_single_transcript` (`ivgs-workers/tasks/stage1_transcript.py:355-372`) tests only `if not refined_text` — a non-empty string of any content passes — and `_update_transcript` (`:241`) PATCHes whatever it is. There is no length ratio against the input, no refusal-phrase check, no floor. **A v8-adjacent validator gap: ROWED, NOT BUILT HERE** (operator ruling 2026-08-29). The obvious shape is a ratio guard plus a refusal-phrase check at the extraction seam, which is non-frozen; it needs its own measurement of what ratio is normal across real refinements before it can refuse anything |
| ⛔ **RC-Q4** | **PER-SCENE PRESENTER SELECTION DOES NOT EXIST, AND THE FOUNDATION ASSUMES IT DOES. PHASE-5 CANDIDATE, OPERATOR DECISION.** `Instructional_Design_Foundation_for_IVGS_2026-08-29.md` §4's modality table gives `talking_head` a row (*"events 1, 2, 7 and 9 are its natural home"*) and §8 assigns it to scenes 0 and 1. **There is no such `MediaType`**: the enum, the PostgreSQL type and `MEDIA_TYPE_SYNONYMS` all carry exactly `image, video_clip, animation, motion_graphics`. The talking head is a pipeline STAGE that renders once and composites as an overlay across the whole video. ⛔ **And a scene claiming an unknown media type does not degrade — `_validate_storyboard_json` RAISES by design (`ivgs-workers/tasks/stage2_storyboard.py:304-311`) and ONE such scene fails the entire storyboard**, which is what RC-P4 records happening the first time a storyboard model chose a value the enum lacked. ✅ **RULED 2026-08-29: `talking_head` stays OUT of `media_type`; v8 refuses it explicitly by name; RULE 13 routes human/social moments to `image`; the project-level talking-head track continues to serve social presence globally.** Whether per-scene presenter selection should be BUILT is open, and it is a genuine pedagogical capability gap rather than a prompt defect — Foundation §4 is right that social presence serves events 1/2/7/9 |
| ⓘ **RC-Q5** | **`pipeline_checkpoints.version_fingerprint` HAS EXISTED SINCE MIGRATION 0002 AND NOTHING HAS EVER WRITTEN TO IT** (grep over `ivgs-workers`: no hits; `save_checkpoint` sends four keys). WP-IVGS-12 begins populating an equivalent on the design brief — `storyboard_design_briefs.prompt_fingerprint`, carrying either `<type>:db:sha256=…` or `<type>:file:sha256=…` so a run that fell back to the image's `.j2` is distinguishable from one that used a published row. **The checkpoint column itself is still unwritten**; wiring it is a small non-frozen change in `utils/error_handler.save_checkpoint` and is not done here |
| ⚠ **RC-Q6** | **THE PER-SCENE INSTRUCTIONAL HEADER SHIPS AS A TABLE, NOT AS ONE PRE-SELECTED BLOCK.** Foundation §5 shows a single scene's block at the head of that scene's prompt. **No seam carries a scene's IDENTITY into any downstream prompt template**: it exists only inside an f-string built in frozen code (`ivgs-workers/tasks/stage3_images.py:210-216`), and the alternative — overloading `scene_title` — would put pedagogy into the storyboard UI and the composition manifest. So the whole table is delivered, keyed by the scene number the user prompt already names, via `Environment.globals` (stages 3 and 5, whose system prompts are files) and a `jinja2.ext.Extension.preprocess` hook (video, whose template is a constant inside a frozen body). **Closing the gap is one line inside three frozen bodies: ledgered for the M3.3 window**, where those bodies become activities and the edit is free |
| ✅ **RC-Q7** | **THE STAGE-2 LLM CLIENT TIMEOUT WAS 120 s INSIDE A 270/300 s DECLARED POLICY, AND IT KILLED THE FIRST ACCEPTANCE RUN. FIXED.** `WorkerConfig.get_vllm_config_for_stage("storyboard_generation")` returned `self.timeouts.vllm_timeout` — one shared 120 s knob used by every stage — while Appendix C has declared soft 270 / hard 300 for this activity since WP-41 and WP-IVGS-10's addendum made the CELERY limits actually apply. **Nothing ever reconciled the inner client timeout with either.** It did not matter while a v7 storyboard took 55 s. ⛳ **MEASURED 2026-08-29 on the operator's own script against the pinned engine: a v8 Design Contract under grammar-constrained decoding takes 169.3 s** (11,914 prompt tokens, 3,238 completion tokens, `finish_reason=stop`, 9 scenes). Comfortably inside the policy; nowhere near inside 120 s. Four consecutive `vLLM timeout: All vLLM endpoints timed out` before it was understood. **This is RC-P17's shape one layer down and it is fixed the same way: DERIVED from the declared policy** (`soft − 30 s` headroom = 240 s), not transcribed from it, because a transcription is *"an accurate mirror with no authority"*. The client must lose the race to Celery: a `VLLMTimeoutError` is named, retryable and logged, while `SoftTimeLimitExceeded` kills mid-write and strands the job row `running` (RC-P16). ⚠ **The other nine stages still share the 120 s knob** and none of them has been measured against its own policy — RC-P18's question, one level down |
| ⛔ **RC-Q8** | **`scripts/save-image-artifact.sh` SILENTLY SHIPS STALE BYTES WHEN A TAG IS REBUILT, AND `DEPLOY VERIFIED` CANNOT SEE IT.** The script skips the save when an artifact of that name already exists (`if [ -s "$OUT" ]` — a deliberate ruling, P1.4j, to stop MANIFEST.txt filling with duplicate lines). ⛔ **The name is derived from the TAG.** So a tag rebuilt during a session — which is what every mid-deploy fix produces — re-saves nothing, `docker load` puts the OLD image back under the same tag on every remote node, and `verify-deployed-image.sh` reports **DEPLOY VERIFIED** because it compares the tag. Measured 2026-08-29: node-01 ran image `e9c1001a` while nodes 02/03/04 ran `aa89c778` under the identical tag, and the 120 s timeout the fix had removed was still live on all three. Caught only by comparing `.Image` IDs across nodes and by reading a config value out of each container. ⚠ **The script DOES print `artifact already present, skipping save` — I tailed past it**, so this is a reporting-affordance defect as much as a script one. Candidate fixes, none taken here: refuse when the local image ID differs from the banked one; or include the image ID in the artifact name. **`docker inspect --format '{{.Image}}'` compared ACROSS nodes is the only check that catches this**, and it belongs in §6.1a beside the tag assertion |
| ⛔ **RC-Q9** | ⛔ **THE HEADLINE ACCEPTANCE FINDING. THE DESIGNER PARAPHRASES THE OPERATOR'S LEARNING OUTCOMES AND SILENTLY DROPS ONE — AND NOTHING CAN CURRENTLY CATCH IT.** Three consecutive generations, 2026-08-29, on three genuine ABCD outcomes supplied through the WP-64 form field. **All three emitted TWO outcomes, not three**, and all three REWROTE the two they kept: *"Given two 2-digit numbers written in column form, the learner will compute their product using the standard column algorithm, producing both partial products with correct carries and a correct final sum"* came back as *"The learner can multiply two double-digit numbers."* — marked `measurable: true`, with `proposed_refinement: null`. **LO-3 vanished entirely and was not declared in `dropped_beats` either.** The v8 system prompt says *"COPY EACH ONE INTO `outcomes[].text` EXACTLY AS WRITTEN"* and *"You never edit the owner's words"*; the schema cannot enforce it, because a paraphrase is a valid string. ⛔ **THE GATE SHOWS NOTHING WRONG**: the matrix is drawn against the paraphrase, every outcome the CONTRACT declares is served, and the operator has no way to see that the spine of their course was reworded and shortened. **This is RC-P14's shape at the outcome layer and it is worse, because the outcomes are what everything downstream aligns to.** ⛳ **The check that would catch it is cheap and objectively decidable and does NOT exist**: compare the contract's `outcomes[]` against `projects.learning_outcomes` — count first, then text — and HARD-REFUSE a missing or reworded operator outcome. `design_review.review()` cannot do it today because it is never given what the operator actually wrote. **ROWED, NOT BUILT** — WP-IVGS-12 is at its Task-7 stop point, three runs is a measurement rather than one, and whether the answer is a validator check, a prompt change, or both is the operator's ruling |
| ⛔ **RC-Q10** | **A STORYBOARD RE-RUN LEAVES SURPLUS SCENE ROWS, AND THE DESIGN REVIEW IS THE FIRST SURFACE THAT MAKES IT VISIBLE.** Pre-existing and self-documented: `StoryboardService.create_scene` upserts by `scene_index` and *"a re-run that produces FEWER scenes than the project already has leaves the surplus rows behind ... this method sees one scene at a time and cannot know the new total"*. Measured 2026-08-29: generation 1 produced 13 scenes, generation 2 produced 8, and the gate then showed **13 rows — five of them from a design that no longer exists**, carrying no declarations and therefore drawing **24 hard refusals** where the generation's own contract drew **1**. ⛳ **Before this package the surplus was invisible** (an extra thumbnail among thumbnails); the design brief makes it loud, which is an improvement that reads as a regression. **Consequence for measurement: a regenerate-on-the-same-project gate reading is contaminated**, and WP-IVGS-12's acceptance was therefore scored on each generation's own emitted contract, stated as such. The fix is the whole-storyboard write that stage 2 does not make — already ledgered for the Temporal cutover |
| ✅ **RC-Q9 CLOSED BY STRUCTURE, 2026-08-29 (WP-IVGS-12b)** | **THE MODEL IS NO LONGER ASKED TO TRANSCRIBE THE OUTCOMES, SO IT CANNOT PARAPHRASE THEM.** Code parses `projects.learning_outcomes` (`shared/design/outcomes.py`, reversibility proven over an 11-case corpus), assigns positional ids `LO-1..n`, and `DesignBriefService._outcomes_from_the_project` injects the operator's text VERBATIM with `authored_by: "operator"`. **`outcomes[]` is GONE from the model's schema**; it emits `outcome_notes` keyed by the real ids (bloom, measurable, `proposed_refinement`) and Foundation §2's refinement path survives intact — proposed against an id, beside text the model never touched. ⛳ **`serves_outcomes`, `evidence_map` and `outcome_notes` are closed by a PER-REQUEST enum built from the project's own ids, measured ENFORCED on the pinned engine** (given `["LO-1"]` and ordered in the prompt to serve LO-1/2/3, the model emitted LO-1 only). **PROVEN over three consecutive generations, 2026-08-29:** all three outcomes present verbatim every time, `invented ids: NONE` every time, `outcome_notes` = 3 of 3 on two of three. The Task 1(d) byte-compare belt (`OUTCOMES_COUNT_DRIFTED` / `OUTCOMES_TEXT_DRIFTED`) never fires **and cannot**, by construction — it exists so a future regression that routes outcome text back through a model is LOUD. Evidence: `dev/workpackages/reference/wpivgs12b-acceptance/` |
| ⛔ **RC-Q9b, NEW AND OPEN — THE TRIAD IS STILL NOT SATISFIED, FOR A DIFFERENT REASON** | With the spine fixed, the acceptance still does not reach zero hard refusals: **2, 2 and 3** across three generations, dominated by **`OUTCOME_UNASSESSED`** — the designer SERVES an outcome and never has a scene ASSESS it. Generation 3 additionally left **LO-3 unserved entirely**. ⚠ And `EVIDENCE_MAP_DISAGREES` flagged on nearly every outcome of every generation: the model fills `evidence_map` with a claim its own scene events do not support. ⛳ **This is a better failure than RC-Q9 was** — the gate is refusing for a real pedagogical reason against the operator's own words, rather than drawing a matrix over a paraphrase — but it IS a failure and the acceptance criterion is not met. **The structural options, none taken here:** derive `evidence_map` from the scenes instead of asking for it (losing the cross-check); or close it so an outcome with no assessing scene is ungrammatical (a `minItems: 1` per-outcome array, which the runaway measurement says needs a `maxItems` beside it). **ROWED, NOT BUILT** — same restraint as RC-P2/RC-P14, and the same reason: the ruling is the operator's |
| ✅ **RC-Q8 CLOSED, 2026-08-29 (WP-IVGS-12b)** | **THE ARTIFACT'S IDENTITY IS NOW NAME + IMAGE DIGEST, AND SKIP-IF-PRESENT IS DIGEST-CONDITIONAL.** ⛳ **The digest is a SIDECAR, not part of the name, and the shape was argued from every consumer**: `artifact_path_for`/`artifact_require` resolve from an image REF and nothing else — that IS the deploy contract, because a remote node has the tag, does not have the image, and the artifact exists precisely to carry the image it lacks; digest-in-name would force every deploy caller to know the digest before it could find the file. `check-image-artifacts.sh` pins the NAME SHAPE and would go red for every artifact ever banked; `tests_system/test_wp58_retention.py` asserts `artifact_name_for`'s exact output and is doing its job. So the name is unchanged and **the fix is in the SKIP LOGIC, which is where the defect was.** New: `image_local_digest`, `artifact_digest_path_for`, `artifact_banked_digest`; a `.digest` sidecar written on every save; MANIFEST rows gain `image:<digest>`; **a different digest under the same tag is a REFUSAL naming both**, proven twice live; a present artifact with no recorded digest is RE-SAVED rather than adopted, because stamping a digest onto unverified bytes would make a lie checkable. `check-image-artifacts.sh` reports artifacts with no recorded digest (93 of 95 at the time of writing) **without failing** — a permanently-red gate is the mistake its own header records WP-56 closing. ⛳ **WHICH DIGEST WON, AND WHY:** for `v5.37.0-design-core` it is **`sha256:e9c1001a…`** — the only build carrying the RC-Q7 client-timeout fix, the bytes all four nodes were running, and the bytes in the bank. Proven by round-trip: `docker load` of the banked artifact restored `e9c1001a` under that tag on node-01. `aa89c778` predates the fix and is retired. The fleet was then moved to **`v5.37.1-outcomes-by-code` / `sha256:6f5bcf93…` on all four nodes, verified by IMAGE ID, not by tag** |
| ⛔ **RC-Q11, NEW — `PromptType` WAS ADDED TO THE DATABASE AND NOT TO THE ORM. WP-68's DEFECT, REPEATING.** | Migration 0047 (WP-IVGS-12) added `transcript_refinement_system` and `storyboard_generation_system` to the PostgreSQL `prompt_type` type. `ivgs-api/app/models/prompt.py` typed its members **by hand** and did not gain them. The INSERT succeeded, both rows published, and **the next SELECT that touched one raised** `LookupError: 'storyboard_generation_system' is not among the defined enum values` — exactly what that column's own comment has warned about since WP-64, and exactly what `MediaType`'s docstring records happening to `motion_graphics`. ⚠ **It hid for a whole package** because nothing read those rows back through the ORM until 12b's publisher looked for a version to supersede; the orchestrator reaches them through the API's own filtered query. ✅ **FIXED THE WAY `MediaType` WAS: one list.** `shared.models.enums.PROMPT_TYPES` is derived from the enum and `PG_ENUM(*PROMPT_TYPES, …)` consumes it, so a value can no longer exist in one place and not the other. **A warning is not a mechanism** |
| ⛔ **RC-Q12, NEW — MEASURED CONSTRUCT SUPPORT ON THE PINNED ENGINE, AND ONE OF THEM IS A LIVE HAZARD IN SHIPPED CODE** | Probed 2026-08-29 against `sha256:3dbe092e…`. ✅ **A PER-REQUEST `enum` IS ENFORCED** — it differs on every call and cannot ride a cached grammar, and it held: given `["LO-1"]` the model emitted LO-1 only, having been told to serve LO-1, LO-2 and LO-3. ✅ **`maxItems` IS ENFORCED**, on string-enum arrays and object arrays alike. ⛔ **`minItems` WITH NO MAXIMUM IS A RUNAWAY**: constrained decoding offers an infinite LEGAL continuation and the model takes it — `["LO-1","LO-3","LO-3","LO-3",…]` to the token limit, `finish_reason=length`, nothing parseable. **The v8 contract WP-IVGS-12 shipped had `minItems` and no max on `serves_outcomes`, `source_refs` and `scenes`**; it happened to terminate, and the hazard was live. Every array in the contract now carries a `maxItems`. ⛔ **`uniqueItems` IS REFUSED: HTTP 400 `Grammar error: Unimplemented keys: ["uniqueItems"]`.** ⛳ **AND NOTE THE CONTRAST WITH RC-Q1 — an unimplemented GRAMMAR key is refused LOUDLY while an unknown BODY member is discarded SILENTLY.** Two failure modes, one engine, and only one of them tells you. Probes banked at `dev/workpackages/reference/wpivgs12-prompt-stack/` |
| ⓘ **RC-Q13, NEW — THE TWO-DOORS IMPORT DIAGNOSTIC IS PERMANENT, AND THE CAUSE IS STILL NOT ESTABLISHED** | Promised by WP-IVGS-12 and delivered. `celery_app.on_worker_init` logs `sys_path`, the derived anchor and a full traceback on `design_contract_observer_registration_failed`. **The unexplained part, restated so nobody assumes it was understood:** a MODULE-LEVEL `sys.path` anchor derived from `__file__` did NOT make `design_core` importable in the worker, and the SAME derivation placed INSIDE the `worker_init` handler did. Both are now in place, the symptom is gone, and **why the module-level one was insufficient was never determined.** The class is WP-IVGS-10's addendum: `python3 -c` puts the cwd on `sys.path` and a console-script entry point puts `/usr/local/bin` there instead, so every check run through the wrong door passes |

---

# §12i-watch — WP-IVGS-12i: mechanical refusals are repaired by code, and declared (2026-08-30)

Operator's Phase-1 watch, RC-R batch, against the live project `680d9e4c-608b-488a-9270-9b4317a7f693`
("sunday's test", 19 scenes, gate open). ⛳ **The operator's project was READ and never written.**
Every write is on `0840e6c2-ab35-4206-bced-2eac9bde6dd5`, a throwaway created for this watch and
deleted through the WP-59 flow.
Report: `dev/workpackages/reports/WP-IVGS-12i-AUTO-REPAIR-report_2026-08-30.md`.
Evidence: `dev/workpackages/reference/wpivgs12i-evidence/`.

**THE PRINCIPLE, as ruled (operator, 2026-08-30):** a gate refusal is either **MECHANICAL** — a
deterministic default fix exists — or **JUDGMENT** — a human must decide. Mechanical refusals are
repaired by code before the gate, **declared, never silently**; judgment findings surface to the
human. No prompt loops: repair is deterministic code plus the proven per-scene authoring call, one
pass, never an LLM retry.

| row | statement |
|---|---|
| ✅ **RC-R4** | **THE AUTO-REPAIR PASS EXISTS, RUNS IN THE PIPELINE, AND DECLARES EVERY REPAIR — AND IT REPAIRED 3 OF 8, NOT 8 OF 8.** `app/services/storyboard_repair.py`, called by `pipeline_orchestrator_v2.handle_stage_completion` at the one seam where "before the gate" means anything: after stage 2 has written its scenes and immediately before `pipeline_paused_at_gate`. ⛳ **PROVEN END-TO-END through the real pipeline, 2026-08-30, on the operator's own script:** 15 scenes, **8 hard refusals in, 3 repaired, 5 repair-refused, 5 out** (`auto_repair_pass_complete`, 15:04:42Z). ⛔ **THE ACCEPTANCE TARGET OF ZERO MECHANICAL REFUSALS WAS NOT REACHED, AND THE REASON IS THE RULING WORKING RATHER THAN FAILING.** The default exit is `media_type := motion_graphics` plus the proven per-scene authoring call, and **WP-IVGS-09f's guard refused five of the eight authorings** — three because the model chose `place_value_split` for narration containing no place-value words (scenes 0, 4, 5: *"Hi! Today, we're going to learn how to multiply two-digit numbers"* is not a place-value step), two because the authored numbers cannot draw the number the narration announces (scene 7: *"the narration announces 4 … but `column_addition_carry{top:230,bottom:92}` never produces 4"*; scene 10 similarly). **Each of the five was PUT BACK — `media_type` reverted — and its original refusal STANDS with BOTH errors named**, per the ruling. ⛳ **So the pass's real product is not a clean gate; it is a TRIAGED one:** eight identical-looking `DELEGATES-TO-WRONG-MEDIUM` rows became three silent fixes and five refusals a human genuinely must decide, each carrying the sentence that says why. **Whether "mechanical, tried, and refused by the guard" should be RECLASSIFIED as judgment is an operator ruling and is NOT taken here** — the counters report `mechanical_before` and the residue separately so either ruling can be applied to the same numbers |
| ✅ **RC-R4 (classification)** | **FOUR MECHANICAL, TWENTY JUDGMENT, AS DATA RATHER THAN PROSE.** `MECHANICAL_CODES` is the only thing that decides, and it holds **every hard refusal `storyboard_completeness` can emit** — `VISUAL_DEMANDS_ON_SCREEN_TEXT`, `NARRATION_TEXT_UNDECLARED`, `MOTION_WITHOUT_TEMPLATE`, `MOTION_CONTRADICTS_NARRATION` — because all four are `DELEGATES-TO-WRONG-MEDIUM` and all four are answered by the one exit the validator's own message names. ⛔ **THE OTHER EXIT THAT SAME MESSAGE OFFERS IS NOT MECHANICAL AND IS NOT TAKEN:** *"set `text_carried_by='narration'` and describe the non-text situation"* requires somebody to REWRITE the description so it stops asking for the digits, which is prose, which is judgment, and which a model asked to produce would make a prompt loop. ⛳ **`design_review`'s sixteen hard codes are ALL judgment**, argued individually in the module docstring in three families: the fix is a scene that does not exist (`OUTCOME_UNSERVED`, `OUTCOME_UNASSESSED`, `OUTCOME_ASSESSED_TWICE`, `PLAN_ENTRY_UNREALIZED`, `SCENE_SERVES_NOTHING`, `EVIDENCE_NEAR_DUPLICATE`); a DECLARATION is missing and there is no default value for "which of Gagné's nine events is this" (`SCENE_NO_EVENT`, `SCENE_BAD_EVENT`, `SCENE_PROVENANCE_UNDECLARED`, `SCENE_CITES_UNKNOWN_OUTCOME`, `SCENE_SOURCED_WITHOUT_REFS`); or the finding IS the belt proving a regression shipped and repairing it would erase the evidence (`OUTCOMES_COUNT_DRIFTED`, `OUTCOMES_TEXT_DRIFTED`). `MOTION_WITHOUT_TEMPLATE` / `MOTION_UNKNOWN_TEMPLATE` / `MOTION_WITHOUT_PARAMS` describe the state the completeness limb already repairs and clear as a consequence; two repairers for one defect is WP-IVGS-09f's "two builders for one payload". **`test_every_hard_refusal_the_classifier_emits_is_classified` fails the day someone adds a fifth refusal kind without arguing it** |
| ✅ **RC-R4 (declaration)** | **`storyboard_design_briefs.system_corrections`, migration 0054, nullable with NO server default.** ⛳ **NULL AND `repaired: 0` ARE DIFFERENT FACTS** — NULL means the pass never ran (every pre-12i brief), a stored row with `repaired: 0` means it ran and found nothing mechanical. A `server_default` would have collapsed the two and made every old brief claim a clean pass. ⛔ **THE FAILED REPAIRS ARE DECLARED TOO, AND FIRST**: a reviewer looking at a scene that is still refused is owed the fact that code TRIED and what it was told, or they will read an untouched refusal as one nobody looked at. Rendered at the gate as a **System corrections** section (`DesignBriefPanel`), above the findings |
| ⛔ **RC-R1 — THE BUTTON DID NOT LIE; IT CRASHED. `POST /scenes/approve` ANSWERED HTTP 500 WITH REFUSALS OUTSTANDING, AFTER RECORDING THE APPROVAL. FIXED.** | Measured 2026-08-30 on the acceptance project: `{"code":"INTERNAL_ERROR","message":"An unexpected error occurred","request_id":"d3ed86f6…"}`. ⛳ **The completeness gate was never the problem** — it has refused correctly with `STORYBOARD_INCOMPLETE` since WP-IVGS-10 Task 3, and that limb is proven working. The 500 comes one step EARLIER: `approve_storyboard` runs `_author_missing_motion_specs` before the completeness check, that helper RAISES `RegenerationError` for a motion scene whose template cannot be authored (by design, WP-IVGS-09f: *"one scene that cannot be drawn is a reason not to start"*), and `_gate_decision` caught `PipelineAlreadyRunningError`, `StoryboardIncomplete` and `ValueError` — **not that**. ⛔ **AND THE GATE DECISION ROW WAS ALREADY WRITTEN** (`gate_decision … decision=approved`, 15:05:39Z) one second before the traceback, so an approval stood on record with a 500 on screen and no dispatch. ⛔ **A 500 SAYS THE SYSTEM BROKE; A REFUSAL SAYS THE SYSTEM REFUSED — OPPOSITE INSTRUCTIONS ABOUT WHAT TO DO NEXT**, and the operator got the wrong one on a storyboard behaving exactly as designed. The log held the whole answer (*"the narration announces 4 … but `column_addition_carry` never produces 4"*) and the surface threw it away. ✅ **FIXED**: `MOTION_AUTHORING_REFUSED`, HTTP 409, carrying the guard's own sentence, proven live (`approve-409-AFTER-rcr1-fix.txt`) |
| ✅ **RC-R1 (the button)** | **THE APPROVE BUTTON IS NOW DISABLED WHILE REFUSALS STAND, FROM THE SAME ARRAY THE BANNER COUNTS.** Before this package the panel said *"14 would be refused"* and *"Approving is refused until these are fixed"* directly above a fully enabled green button. `GateReviewPanel` now computes `refusalCount` from `state.completeness` — **not a second fetch and not the error body**, because two numbers derived two ways will disagree — and renders **"N refusals block approval"** as a sentence under the buttons as well as a tooltip on the disabled one. ⚠ **ONLY `approved` IS BLOCKED**: Reject and Regenerate are how a reviewer ACTS on a refusal, and disabling them would trap the gate closed. The refusal payload also gains a `refusals` integer beside the named code, so a surface never has to parse an English sentence for N. **Proven at both states on the acceptance project: 5 refusals → 409 by name; 0 refusals → `refuse_if_incomplete` passes over all 15 scenes with the 4 soft flags intact** |
| ⛔ **RC-R1 (the OTHER banner, ROWED not built)** | **`DesignBriefPanel` claimed "N design refusals — approving will be refused by name", AND THAT WAS FALSE.** `design_review`'s sixteen hard codes are **not enforced anywhere on the approve path**: `approve_storyboard` runs `_author_missing_motion_specs` and `refuse_if_incomplete` and nothing else. Measured 0 design refusals on both projects today, so nobody has been misled yet — but the sentence was a promise the server does not keep, and it is the same class as the button RC-R1 exists to fix. ⚠ **THE WORDING IS CORRECTED TO WHAT IS TRUE** (*"objectively checkable, and yours to resolve before approving"*); **whether design refusals should BLOCK approval is an operator ruling and was not taken** — making the gate refuse on pedagogical grounds is a change to what the gate IS, not a bug fix |
| ✅ **RC-R2 — PER-SCENE FINDINGS NOW REACH THE WORK SURFACE** | The gate panel lives on the project tab; the scene is changed in the Edit modal on another tab. Until today a reviewer told *"scene 7 delegates written content to a diffusion medium"* had to leave the panel, find scene 7 in a grid of nineteen, and carry the complaint in their head. Now: **a badge on the card** (red for `refuse` carrying the code, amber for `flag`, nothing at all when clean — a badge that is always there stops being read) and **the findings in full, in the server's own words, inside the modal**, above the actions. One array, indexed once on the page, shared by the panel, the cards and the modal, so no two surfaces can disagree about the same scene. `SceneAssessment` gained a stable `code` for this and for the repair pass |
| ⛔ **RC-R2 — THE ONE-CLICK "AUTHOR AS MOTION GRAPHICS" DID NOT EXIST. THE ORDER'S PREMISE WAS WRONG, AND IT IS BUILT NOW.** | The order says it *"remains"*. It never did. What the modal had was **WP-64's "Adapt description"** — an LLM proposal that rewrites the visual description for the medium already selected, saves nothing, and does not author a template. Flipping the dropdown to Motion Graphics and pressing Save has always produced a motion scene with **no template**, which is the `MOTION_WITHOUT_TEMPLATE` refusal and the `Needs template` badge WP-IVGS-09c added to explain it. ✅ **BUILT**: `POST /projects/{id}/scenes/{sid}/author-motion` and an emerald button beside Adapt. **It is the SAME primitive `storyboard_repair` calls** — one mechanism, two surfaces, per WP-45's dispatch rule — and it refuses by name with 409 `MOTION_AUTHORING_REFUSED`, leaving the scene exactly as it was, medium included. This is the manual/override path the ruling reserves for judgment cases |
| ⓘ **RC-R3(a) — THE JOBS TAB NAMES THE FIRST STAGE AND THE LEDGER RECORDS TWO. OPERATOR EVIDENCE, REGISTERED ON THE PHASE-3 LEDGER-READ-MODEL ROW.** | Recovery plan §4 **Phase 3 — "One truth"**, *"Ledger-authoritative read model: job label/duration from checkpoints … Exit test: on a fresh run, Jobs tab matches the ledger at every stage."* **Measured on BOTH projects today.** Live `680d9e4c`: one `render_jobs` row, `job_type = transcript_refinement`, while `pipeline_checkpoints` records **two** complete stages — `transcript_refinement` at 14:06:52Z and `storyboard_generation` at 14:08:09Z. The Jobs tab renders `job.job_type` verbatim (`app/projects/[id]/jobs/page.tsx:203`), and that column is **frozen at job creation**, so **storyboard generation — the stage that produced the artefact the operator is now looking at — has no row at all**, and the run reads as "transcript refinement" for its whole life. (The job row's own span, 14:06:52Z → 14:12:05Z, is 312 s; the label is what is wrong here, not the arithmetic.) The acceptance project `0840e6c2` reproduces it exactly. **This is RC-L8's TYPE column, one level down, and it is the Phase-3 exit test failing on a fresh run.** ⛔ **ROW ONLY. NOTHING BUILT** — the fix is the read model, it is Phase 3, and building it inside a Phase-1 watch would be the scope creep the watch's own "narrow scope" instruction forbids |
| ⓘ **RC-R3(b) — THERE IS NO DESIGN-DURATION ADVISORY, AND THE LIVE PROJECT IS 17:00 AGAINST A DECLARED 5:00. ROWED.** | Measured 2026-08-30: `projects.max_runtime_seconds = 300` on `680d9e4c`, and its nineteen scenes sum to **1,020 s — 17:00, three and a half times the declared budget**. **Nothing anywhere compares the two.** `design_review` has no duration finding of any severity; `gate_service` computes none; `max_runtime_seconds` reaches only the job context and the stage-1 prompt (`project_service.py:494`, `checkpoint_service.py:348`, `stage1_transcript.py:158`). ⛳ **AND IT MUST NEVER BE A REFUSAL. By the standing ruling, duration DERIVES FROM THE DESIGN**: a course that needs seventeen minutes to teach column multiplication needs seventeen minutes, and a gate that refused it would be enforcing a number the operator typed before they knew what the lesson was. So the row is: **an ADVISORY in the design brief — "this design runs 17:00 against a declared 5:00"** — placed where the reviewer is already deciding, blocking nothing. ⚠ **Note the direction of the historical error**: WP-63/WP-IVGS-12 record stage 1 being told to *"align with max_runtime_seconds"* and turning a four-minute script into 1:45. The budget has already once been allowed to eat the content; an advisory is the shape that does not repeat it. **ROW ONLY, NOT BUILT** |
| ⚠ **RC-R5, NEW — THE AUTHORING MODEL PICKS A TEMPLATE THE GUARD'S OWN TRIGGER WORDS FORBID, AND IT DID SO THREE TIMES IN EIGHT.** | Not a defect this package introduced; a defect this package MEASURED for the first time, because nothing before it ever called `author_params_for_scene` on a scene that was not already `motion_graphics`. Three of five repair refusals are the same shape: the model answered `place_value_split`, and `verify_spec_against_narration` refused it because *"template 'place_value_split' needs narration about place value/tens place/ones place, and this scene's words contain none of them"*. ⛳ **THE GUARD IS RIGHT AND THE REFUSAL IS CORRECT** — for scene 0, an introduction, and scene 5, *"Explain why 56 is different from 60 in terms of units and tens"*, the honest answer is that **no template fits and the scene is not a motion graphic at all**. So this may be a prompt defect (the authoring prompt does not tell the model it may say "none of these"), or it may be the classification question RC-R4 leaves open (a mechanical exit that the guard reliably refuses is judgment in practice). **ROWED, NOT FIXED. The ruling is the operator's, and three instances is a measurement rather than one** |

---

# §12i-watch2 — WP-IVGS-12i2: the RC-S batch (2026-08-30)

Operator's second watch, live project `680d9e4c-608b-488a-9270-9b4317a7f693`
("sunday's test") after its regeneration, gate open. ⛳ **The operator's project
was READ and never written by hand.** Every write is on `43c59a2a-917a-4df0-9368-ce5752f1651d`,
a throwaway created for this watch and deleted through the WP-59 flow.
Report: `dev/workpackages/reports/WP-IVGS-12i2-RCS-BATCH-report_2026-08-30.md`.
Evidence: `dev/workpackages/reference/wpivgs12i2-evidence/`.

| row | statement |
|---|---|
| ✅ **RC-S1 — THE SECOND ASSESSMENT WAS A GHOST. STALE ROWS SURVIVED THE REGENERATION; CALL 2 WAS INNOCENT. FIXED AT CAUSE.** | The order named four candidates and the answer is the third. **Measured, read-only:** the ACTIVE contract emits per-LO assess **`{LO-1:[9], LO-2:[12], LO-3:[15]}` — exactly one each**, so neither call 2 nor the merge nor the repair pass produced a second pair. The 14:12 generation wrote **19** scenes; the 15:42 regeneration designed **17**. Rows 17 and 18 have no entry in the active contract and carry `updated_at = 14:12:05` while every regenerated row carries `15:42:37` — and **row 18 is an `assess` serving LO-3**. The gate therefore saw LO-3 assessed by scene 15 (the design) and scene 18 (a design that no longer exists). ⛳ **The validator's own sentence — *"this firing means the structural guarantee has stopped holding"* — was TRUE about the database and FALSE about the contract, which is why it read as a contract-7 regression and was not one.** ⛔ **CAUSE, SELF-DOCUMENTED FOR TWO PACKAGES: `ivgs-api/app/services/storyboard_service.py:92-98`** — *"a re-run that produces FEWER scenes than the project already has leaves the surplus rows behind ... Trimming needs the whole-storyboard write that Stage 2 does not make"* (RC-Q10). ✅ **FIXED: `storyboard_repair.prune_scenes_not_in_design`** reconciles the rows against `scene_designs` on the ACTIVE brief — the design of record by RC-Q18's ruling — and runs BEFORE the repair pass, at the same pre-gate seam. **Index-set membership, not `index >= count`**: a contract with a gap in its indices is trimmed correctly by one and wrongly by the other. ⛔ **It refuses to guess**: no active brief, or no `scene_designs`, prunes NOTHING and records which of the two silences it is, so a pre-v8 storyboard is untouched. Every removed row is declared in full on the brief. ⛳ **ACCEPTANCE, two real pipeline runs on the test project: generation 1 designed 37 scenes and pruned 0; the regeneration designed 16 and pruned 21 — the storyboard is 16 rows, and per-LO assess is LO-1=1, LO-2=1, LO-3=1.** Among the 21 was scene 36, an `assess` serving LO-1: the live defect, reproduced and removed |
| ✅ **RC-S2(a) — ONE THROWAWAY DECLARED DROP DEFEATED THE WHOLE FIDELITY CHECK. THE RULE IS PER-SPAN NOW.** | 12b's rule was `gap >= 400 AND dropped_beats is empty`, and **the second clause is GLOBAL where the first is PER-SPAN**. ⛳ **The fix needed no new bookkeeping and that is the argument for its correctness: a declared drop's span is ALREADY merged into the coverage, so a stretch that survives as a gap is by construction one that no drop declared.** Dropping the global clause therefore IS the operator's rule rather than an approximation of it. New code `UNDECLARED_SPAN_OVER_THRESHOLD`; the message names how many drops were declared elsewhere and states that a drop declared somewhere else is not a declaration about here. ⛳ **CALIBRATED AGAINST BOTH LIVE DESIGNS, AND BOTH REFUSE — CORRECTLY.** WATCH-1 covered **1,622 of 3,138 chars (51.7%)** with 2 drops and left the script's whole *"Step 4: Add the Two Answers"* section, **1,473 chars**, uncovered and undeclared: a real teaching beat, lost silently, and a 1,473-character hole is not span-offset arithmetic. WATCH-2's regen covered **110 of 3,138 — 3.5%** with 1 drop and left **2,968 chars**. **Old rule: zero refusals for both.** ⚠ The 12b tests that pinned the loophole are superseded in place with the change of sense written down, not deleted |
| ⓘ **RC-S2(b) — THE INPUT WAS FULL. THIS IS RC-P15-CLASS VARIANCE. ROWED, NOT TUNED.** | **Call 1 received the whole script.** `refined_text` is byte-identical to `source_text` at 3,138 chars (stage 1 did not paraphrase this time), and the reconstructed call-1 user prompt carries all 3,150 characters of the combined transcript **including the "Step 4" section the design left uncovered**. Checkpoint `total_input_tokens`: **15,547** for watch-1 (19 scenes) and **13,993** for the regen (17 scenes) — both full-scale, the difference explained by a smaller design and a shorter call-2 user message, not by a truncated input. ⛳ **CENSUSES, SIDE BY SIDE.** WATCH-1: `sourced` **13** / `designed` **6**, coverage 51.7%. WATCH-2: `sourced` **6** / `designed` **11**, coverage **3.5%**. Same script, same prompts, same model, same day. **Full input + script-free output = variance, and the order's ruling applies: ROW IT, DO NOT TUNE.** ⛳ **AND RC-S2(a) IS THE ANSWER TO IT** — the fidelity rule does not make the model faithful, it makes an unfaithful design REFUSE, which is the only lever that does not involve prompt-tuning against a measurement of one |
| ✅ **RC-S4 — THE MATHS IS NOW CHECKED, FOR THE HALF THAT IS DECIDABLE. THE OTHER HALF IS ROWED, LOUDLY.** | **The operator's catch**: scene 4's narration — *"To multiply two double-digit numbers, we need to multiply the tens and the units separately"* — is the classic misconception taught as method; a learner computes 23 × 14 as 20 × 10 + 3 × 4 = 212. **Nothing in this pipeline has ever asked whether a scene's content is CORRECT**, only whether it is declared, depictable or consistent with its template. ✅ **BUILT: `shared/design/equations.py`**, a deterministic lint over every scene's narration that parses **complete** arithmetic claims — both operands, the operation and the result, in prose or symbols — and HARD-refuses any false one (`NARRATION_ARITHMETIC_FALSE`), naming the scene and quoting the claim with what it computes to. ⛳ **CALIBRATED, AND THE CALIBRATION IS ITSELF A FINDING: the operator's uploaded script carries 17 complete claims, ALL 17 TRUE, and BOTH live designs carry ZERO.** The lint has nothing to bite on precisely because the designs abandoned the script's arithmetic. Proven live on the test project: a seeded *"4 times 3 equals 13"* refuses with `computed: 12, stated: 13`; the same sentence made true returns zero. ⛔ **AN INCOMPLETE STATEMENT IS NEVER A CLAIM** — *"Now multiply 4 times 3"*, *"write the 2 and carry the 1"*, *"our first answer is 92"* all parse to nothing, and that false-negative bias is the right one for a check that hard-refuses |
| ⛔ **RC-S4 (the method-prose half) — GENERATED-CONTENT METHOD CORRECTNESS STAYS AT THE HUMAN GATE UNTIL THE L7 CHECKER (M3.3). SCENE 4 IS THE DRIVING EVIDENCE.** | *"Multiply the tens and the units separately"* contains **no numerals and states no equation**. Deciding it is wrong requires knowing what the method computes — a semantic judgement about generated prose, not arithmetic. ⛔ **A lint that scored method prose would be worse than none: it would let a reviewer believe the maths had been checked.** `test_the_method_prose_half_is_NOT_caught_and_that_is_declared` pins the limitation so no future package can read the presence of an arithmetic check as a guarantee. ⛳ **AND NOTE WHERE THE PRIMARY GUARD ACTUALLY IS: RC-S2(a). A design anchored to a correct script cannot state a wrong method** — scene 4 exists because the regeneration used 110 characters of a 3,138-character script and invented the rest. Fidelity is what stops the invention; the lint catches only the arithmetic |
| ✅ **RC-S3 — THE WITHIN-LO BELT NOW SEES PAIRS THAT CONTAIN NO ASSESSMENT. FLAG-LEVEL, PER THE RULING.** | 12h's belt is ASSESSMENT-anchored and its own docstring names the residue: *"The practice is NOT compared against the worked examples here."* So the live regen's LO-2 `guide` scene 10 and `practice` scene 11 — **byte-identical narration**, *"Can you identify the units and tens in the number 45?"* — were invisible to it, as were LO-1's 6/7/8 and LO-3's 13/14. **Three duplicated pairs and the gate said nothing.** ✅ `SAME_OUTCOME_NEAR_DUPLICATE` compares **any** two scenes serving one outcome, **using the same `duplication_verdict` and the same calibrated thresholds** so a retune moves both limbs together. ⛳ **HARD ONLY WHERE 12h's RULING ALREADY MADE IT HARD**: any pair containing an `assess` is left entirely to the hard limb — no flag beside a refusal about the same two scenes, because one complaint per sentence is the whole point. Elsewhere it flags: two `guide` scenes restating one question are usually a defect and sometimes deliberate repetition for an anxious nine-year-old, and that is the reviewer's call. **Proven on the acceptance regen: 9 flags over guide/practice pairs.** ⚠ **AND ONE DEFECT OF MY OWN, CAUGHT ON THE ACCEPTANCE RUN AND FIXED**: the limb first shipped reusing `duplication.explain`, whose sentences begin *"the assessment"* — so the live gate reported *"the assessment restates that scene"* about a `guide`/`practice` pair in which neither scene is an assessment. A shared helper whose wording fits only one caller is worse than two sentences; pinned by a test |
| ⓘ **RC-S5, NEW — `storyboard_design_briefs.prompt_fingerprint` IS NULL ON BOTH LIVE DESIGNS.** | Noticed while reading the briefs for RC-S2 and recorded rather than chased. RC-Q5 introduced the column so a reader could tell a run that used a published prompt row from one that fell back to the image's `.j2`, carrying `<type>:db:sha256=…` or `<type>:file:sha256=…`. **Both of the operator's briefs carry `NULL`**, so that distinction is currently unavailable for exactly the two designs this watch is about. Not chased here: it changes no verdict in this batch and the fix is in the worker's capture path, which nothing else in this package touches. **ROW ONLY** |

---

# §12i-watch3 — WP-IVGS-12i3: stage-complete means approvable (2026-08-30)

Operator's ruling, now the invariant: **a correctly completed storyboard stage
arrives at the gate with ZERO mechanical refusals — only judgment flags.** And
its amendment, binding, which governs how that is reached: **CONTENT MUST NOT BE
LOST — dilution to pass validation is a worse defect than a refusal.**

Live project `680d9e4c` read-only throughout. Proven on `43c59a2a`, deleted via
WP-59 at the end.
Report: `dev/workpackages/reports/WP-IVGS-12i3-RCT-EXITS-report_2026-08-30.md`.
Evidence: `dev/workpackages/reference/wpivgs12i3-evidence/`.

| row | statement |
|---|---|
| ✅ **RC-T1 — THE REPAIR PASS NOW HAS THREE EXITS, AND THE RULED ORDER IS A SET OF CONDITIONS RATHER THAN A SEQUENCE OF ATTEMPTS.** | **(a)** author the whole scene as `motion_graphics` *where the template fits the whole narration*; **(c)** SPLIT *where the narration mixes content and context*; **(b)** redescribe *only where the text demand is incidental*. ⛔ **THE FIRST CUT GOT THE ORDER WRONG AND THE ERROR WAS INSTRUCTIVE**: it attempted (a) first and fell through to (c) only when authoring REFUSED. On the operator's own opener — *"Hi! Today… That might sound tricky, but don't worry… By the end, you'll be able to solve a problem like 23 times 14 all by yourself"* — **exit (a) SUCCEEDS**: the words carry "multiply", 23 and 14, so a `column_multiplication_step` is authored and the guard passes it. The result would be a warm welcome to an anxious nine-year-old rendered as an animated column sum, produced by a repair reporting success. *"Fits the whole narration"* is the test, and a mixed narration fails it, so a mixed scene now goes **straight to (c)**. Pinned by a test whose authoring stub deliberately SUCCEEDS |
| ✅ **RC-T1(b) — REDESCRIPTION, NARROWED BY THE AMENDMENT, WITH A POST-CHECK THE MODEL CANNOT TALK PAST.** | One bounded call, `response_format` + `json_schema` + `strict: true` (RC-Q1's mechanism of record — `guided_json` is accepted with HTTP 200 and silently ignored on this fleet), emitting ONE key, followed by the **deterministic post-check: the result is fed back through the very extractor that produced the refusal, and a single surviving demand DISCARDS it**. No retry — the check, not a second attempt, is what makes a bounded call safe. ⛳ **AND LEGALITY IS DECIDED BEFORE ANY MODEL IS CALLED, from the design contract itself**: `instructional_event ∈ {present, guide, practice, assess}` over narration stating written or numeric content ⇒ **FORBIDDEN**, because Foundation §4 reserves on-screen text for *"labels, symbols, and the worked math itself — which narration cannot carry"*. A `media_rationale` that says the learner must SEE it forbids it too: if the designer wrote it down, it is not incidental. Proven live — the intro's *"whiteboard showing the multiplication problem 23 x 14"* became *"a two-row layout, the top row having two placeholders and the bottom row having one, all still empty"* |
| ✅ **RC-T1(c) — THE SPLIT, AND THE THREE THINGS IT GUARANTEES.** | Narration partitioned **at sentence boundaries by code**, using the same `referents` extractor the refusal used; the digit half becomes a `motion_graphics` child authored by exit (a)'s primitive; the context half keeps the parent's medium. **No new judgment and no new model call beyond the one authoring ask.** ⛳ **Both children carry the parent's `serves_outcomes` and `source_refs`, so their spans reunite to the parent's by construction — which is why a split cannot lower fidelity.** Durations split by narration length and sum to the parent's. ⛔ **`assess` lands on ONE child only** (the context half becomes `guide`), so RC-S1's per-outcome invariant survives a split rather than being re-broken from inside the repair. ⛳ **AND TWO INTERACTIONS THAT WOULD HAVE UNDONE IT SILENTLY, BOTH CAUGHT AND CLOSED: (1)** the child is written into the ACTIVE BRIEF's `scene_designs` and every later index shifted, because RC-S1's own prune deletes any row the design of record does not contain — a child left out would have been deleted by the very next pass; **(2)** migration 0048's provenance XOR refused the first cut, which marked every child `designed` while copying the parent's spans. **The constraint was right and the code was wrong**: a split MOVES sentences, it does not write them, so the child inherits the parent's provenance exactly. **Two splits exercised live**, including the intro-with-digits shape the order names |
| ⛔ **RC-T1(c) — AND THE SPLIT DOES NOT FINISH BY ITSELF. MEASURED ON THE ACCEPTANCE RUN.** | A split moves the digits out of the NARRATION and leaves the DESCRIPTION exactly as it was. Scene 6 narrated *"Do not worry, this is easier than it looks. Now multiply 4 times 3…"* under *"A worksheet showing the multiplication problem and the calculations."* — so after a perfectly good split **the parent went on refusing and the stage failed over a scene the pass had just repaired**. ✅ The context parent is now re-examined **once**, and that is not a loop: its narration CHANGED, and the legality test now reads a context-only narration, which is exactly the case where a leftover text demand is incidental. One further exit-(b) call at most, never the same call twice |
| ✅ **RC-T2 — THE STAGE-COMPLETE INVARIANT IS ENFORCED, AND IT FIRED ON THREE REAL GENERATIONS.** | After the pass, mechanical refusals must be ZERO. A scene surviving all three exits **fails the stage**: the job is marked `failed` with every surviving scene named and **every exit's own refusal sentence quoted**, including why exit (b) was forbidden. ⛳ **The rows are left exactly where they are, deliberately** — the operator's remedy is to regenerate OR edit, and deleting the storyboard would remove the second. What changes is the JOB, so the Jobs tab and the gate agree instead of a green job sitting beside a gate full of red. ⛳ **FIDELITY IS PART OF THE SAME INVARIANT**: coverage is measured before and after over the same script and the same merge the gate uses, and **a drop is itself a stage failure**. Measured 40→40, 120→120, 170→170 across the acceptance runs — never lowered. ⚠ **A defect of my own, caught by the acceptance and fixed**: the failure message first derived survivors from "corrections that did not apply", so it printed *"1 scene(s) survived"* above an **empty list**. Survivors now come from the re-validation, so the count and the names are one fact |
| ✅ **RC-T3 — THE PLACE-VALUE LEXICON WIDENED ON THE FOUNDATION'S OWN WORDS; THE THREE RESIDUE SCENES RE-RUN.** | Scenes 9/10 narrate *"Can you identify the unit numbers and 10's in the number 45?"* — a place-value question in the operator's own words — and were refused because the guard wanted the token "place" beside the place's name. **Foundation §3, the Gagné event-3 row, names them itself: *"ones/tens place value; single-digit facts"***, so `ones`, `tens` and `unit` are added and the citation is checked by a test that reads the Foundation file. ⛔ **REFUSED, WITH REASONS: `digit`/`number` (a necessary condition everything satisfies is not a check), any script literal, and `hundreds`/`thousands` (defensible but unmeasured — widen when a run produces one).** ⚠ **The honest cost, stated: this is a NECESSARY condition, so widening weakens it** — *"now multiply by the tens digit"* now also satisfies `place_value_split`. Accepted, because the alternative is refusing a scene for using the Foundation's own words, and the producibility limb is untouched. ⛳ **RE-RUN: scenes 9/10 now author under (a) — they came back `motion_graphics` on the next generation. Scene 1, an `objective` scene with no place-value content at all, is STILL refused by (a) and takes (b), which is the correct split of the three** |
| ⛔ **RC-T4, NEW — THE SAME LEXICON GAP EXISTS IN THE MULTIPLICATION TRIGGER LIST, AND I DID NOT TUNE IT.** | Acceptance run C failed on scene 4: *"For example, 3 x 4 means 3 groups of 4."* Exit (a) refused because `_MULT_WORDS` is `(multiply, multiplying, times, digit, placeholder, zero)` and the narration says **"x"** and **"groups of"** — the multiplication SYMBOL and the standard primary-school phrase, both Foundation-general. Exit (b) was correctly forbidden (a `present` scene over digit narration), exit (c) did not apply (no context sentence), so the scene survived and the stage failed. ⛔ **NOT WIDENED HERE, ON PURPOSE.** `"groups of"` is a safe substring, but adding vocabulary *because one generation failed on it* is precisely the tuning RC-T3 forbids; and `x`/`×` cannot be added at all under the current **plain-substring matcher** (`w in words` would fire on "example", "next", "six"), so carrying the symbol needs the matcher to become word-bounded — a change to every template's behaviour that deserves its own measurement. **ROWED with the measurement, for an operator ruling** |
| ⛔ **RC-T5, NEW — THE INVARIANT NOW MAKES A PRE-EXISTING TRUTH VISIBLE: A CLEAN GENERATION IS NOT YET RELIABLE ON THIS SCRIPT.** | Four generations through the full pipeline, 2026-08-30, all with the three exits deployed: **one reached ZERO mechanical refusals (13 scenes, 4 → 0, all exit (a)) and three failed the stage** — run B on `MOTION_CONTRADICTS_NARRATION` where re-authoring invented a different operand (`bottom=42` for a spec that had invented `bottom=280`), run C on RC-T4's multiplication lexicon, run D with two survivors. ⛳ **THIS IS NOT A REGRESSION AND NOTHING GOT WORSE.** Before this package those same storyboards reached the gate as green jobs carrying red refusals; the stage now says so. **The residue is dominated by the AUTHORING MODEL choosing a template whose trigger vocabulary its own narration does not match** — RC-T3's shape, recurring — which is why RC-T4 is rowed rather than patched. ⚠ **The order's acceptance ("the gate must show mechanical refusals ZERO") was met on one run of four and is stated as such rather than rounded up** |
