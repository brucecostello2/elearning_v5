...
`rollback_points.id` is also `String(36)` in migration 0001. The ORM model should be checked to see if it expects UUID. If so, add a similar type-conversion migration.

**Action for porting agent:** Check `app/models/rollback_point.py` (or wherever the rollback model is defined). If the ORM defines `id` as `UUID(as_uuid=True)`, include a type fix. If it uses `String`, skip.
...
| *(new, if needed)* | *Bonus 3.2: fix rollback_points.id type* | **0026** | `0026_fix_rollback_points_id_type.py` | `"0025"` |
...
git checkout -b feature/sandbox-reconciliation origin/main
...
# Push to remote
git push origin feature/sandbox-reconciliation
...
- [ ] All original sandbox tests pass (153 tests baseline)
