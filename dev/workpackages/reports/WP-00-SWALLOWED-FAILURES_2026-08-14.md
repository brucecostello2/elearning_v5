...
| 20 | `ivgs-workers/tasks/stage1_transcript.py:355-372` (the emptiness test) and `:241` (the PATCH) | **A MODEL'S CHAT REFUSAL, WRITTEN OVER THE OPERATOR'S UPLOADED SCRIPT AND RECORDED AS A SUCCESSFUL REFINEMENT** | Open — **added 2026-08-29 by WP-IVGS-12, register row RC-Q3, operator ruling: ROW IT, DO NOT BUILD IT HERE.** Project `0361c667-272b-4f66-9a08-de791f1f92a6`: `transcripts.refined_text` is *"Sure! Please provide the raw transcript you'd like me to refine."* — **64 characters over a 540-byte upload**, job green. ⛔ **The only check on the model's output is `if not refined_text` (`:368`)** — a non-empty string of ANY content passes, and `_update_transcript` then PATCHes it over the column that also holds the upload (see RC-Q2: there was no second copy until migration 0046). **Nothing anywhere asks whether the output is a transcript at all**: no length ratio against the input, no refusal-phrase check, no floor. This is the lookup-miss shape of instance 19 applied to an LLM: not answering became "this is the lesson". The fix belongs at the non-frozen extraction seam and needs a measurement of the normal input:output ratio across real refinements before it may refuse anything. |
