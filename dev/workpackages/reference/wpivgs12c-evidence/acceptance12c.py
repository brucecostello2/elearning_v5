"""WP-IVGS-12c Task (5): the acceptance criterion, third attempt.
...
print(f"evidence_map bound: {json.dumps(SCHEMA['properties']['evidence_map']['properties'][IDS[0]])}\n")
...
def score(n, obj):
    ...
    print(f"    empty evidence arrays  : {[k for k,v in em.items() if not v] or 'NONE'}")
    ...
    return {"gen": n, "scenes": len(scenes), "notes": len(notes),
            ...
            "invented": invented, "evidence_map": em,
...
json.dump(contracts, open(f"{S}/12c-contracts.json", "w"), indent=2)
json.dump(results, open(f"{S}/12c-results.json", "w"), indent=2)
...
print(f"  every evidence entry non-empty     : {ok_ev}")
