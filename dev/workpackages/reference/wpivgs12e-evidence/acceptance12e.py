"""WP-IVGS-12e Task 2: the acceptance criterion, FIFTH attempt.
...
def score(n, obj):
    ...
    # ⛔ The order asks for the assess scenes' SHAPE, not just their count:
    # pose cold / hold / reveal is what v5 defines, and a scene labelled
    # `assess` that narrates the method is a `guide` wearing the label.
    for s in scenes:
        if s["instructional_event"] == "assess":
            raw = next((x for x in obj["scenes"]
                        if x.get("scene_index") == s["scene_index"]), {})
            print(f"    ASSESS scene {s['scene_index']} "
                  f"({s['media_type']}, {raw.get('duration_seconds')}s) serves {s['serves_outcomes']}")
            print(f"        narration: {str(s['narration_text'])[:190]}")
            print(f"        visual   : {str(raw.get('visual_description'))[:190]}")
            gp = raw.get("generation_params")
            if gp:
                print(f"        params   : {json.dumps(gp)[:150]}")
    print(f"    media : {mt}")
...
json.dump(contracts, open(f"{S}/12e-contracts.json", "w"), indent=2)
json.dump(results, open(f"{S}/12e-results.json", "w"), indent=2)
...
ok_realized = all("PLAN_ENTRY_UNREALIZED" not in (r.get("refusals") or [])
                  and not r.get("TRUNCATED") for r in results)
ok_app = all(any(e in ("practice", "feedback", "assess") for e in (r.get("events") or []))
             and not r.get("TRUNCATED") for r in results)
n_assess = [sum(1 for e in (r.get("events") or []) if e == "assess") for r in results]
...
print(f"  application-bearing 3/3            : {ok_app}")
print(f"  `assess` scenes per generation     : {n_assess}   (12d was [0, 0, 0])")
