...
- `dev/workpackages/reports/WP-70-CONSUMER-FIXES-1-report_2026-09-05.md` — eight of the nine WP-69 §2 one-line defects fixed, one commit each, every fix with a test that failed on the pre-fix tree (S11, S12, S6, S5, S4 as a new `POST /retention/run`, S7, S8, S10); **S13 skipped by the order's own §3 rule** — `storyboard_scenes` has no state column, so `status` needs a migration (Decision D-1). Consumer index rebuilt on the branch head: **8 definite rows removed, 0 added**; the 3 surviving rows are S13's. ivgs-api suite: see the report §4; frontend `test:logic` 117/119 with the same two pre-existing failures. Follow-up for the operator: S13's migration. **v2 (same day, same branch, three more commits):** S5+N3 — the socket now carries `?token=` from `useWebSocket.ts`; S7+N4 — reorder body is `items[{id, sequence_order}]`; D-2 — the Assets page sends `asset_type` from the MIME type. Index unchanged in substance (8 definite removed / 0 added, S13's 3 remain). Frontend `test:logic` 120/122, the same two base failures; ivgs-api 1868 passed / 1 failed, the same 0055-only deletion-map failure the test DB causes, 0 new. **v3 (operator rulings, four more commits):** D-7 one name for the DLQ timestamp (`created_at`, alias field removed); D-8 the DLQ detail view reads the API's names (five fields remapped, nothing deleted); D-6 the retention task name is a literal the index verifies; S13 interim — the storyboard status filter and counters hidden behind `SCENE_STATUS_PRODUCER_EXISTS = false` until a producer exists. Index: 8 definite removed / 0 added, S13's 3 remain, suspect added now exactly 1 (the deliberate test row). Frontend `test:logic` 123/125, same two base failures; ivgs-api 1869 passed / 1 failed, the same 0055-only deletion-map failure the test DB causes, 0 new. Branch `wp-70-consumer-fixes-1` off `origin/main`, HELD; nothing deployed.

## Reports filed this session — WP-74 (2026-09-05, CI baseline)

- `dev/workpackages/reports/WP-74-CI-BASELINE-report_2026-09-05.md` — every suite run at `9c406e8` and inventoried by cause in `dev/audit/test_baseline_2026-09-05.md` (the deliverable): ivgs-api **1870/0** on the rebuilt 0054 test database, workers 988/18/48/15, scheduler 52/15, renderer 24/0/2, backup-worker 4/0, tests_system 193/12/15/30, frontend 123→**125**/125. Inherited: workers 33, scheduler 15, tests_system 42 (node-01 only), one **product defect** among them (`Stage6Input.scene_audio_refs` min length, frozen tree). CI Python test and lint jobs **re-enabled** with a named allowlist and a plugin that fails on any non-inherited failure or stale entry; lint is an inherited state, not green. Three order premises stale (passlib, `tests_system` hosts, `test_stage3.py` — all already fixed by WP-52). Branch `wp-74-ci-baseline`, HELD; nothing deployed; production DB untouched.
