...
**Fleet.** node-01 = CPU hub (Postgres / API / Redis / SeaweedFS / scheduler / beat / `celery-worker-default` / `-composition`). node-02 = LLM-only. node-03 = video-only. node-04 = image + TTS + talking-head (96 GB RTX PRO 6000). **CORRECTED 2026-08-25 (WP-53).** This read "node-05 and node-06 remain OFFLINE" and "node-06's card was physically swapped to an RTX 6000 96 GB". Measured: node-05 is ONLINE (RTX PRO 5000 Blackwell, 48935 MiB) but **out of service since 2026-08-25 for a RAM fault**; node-06 is ONLINE but **unprovisioned**, carrying an **NVIDIA GeForce RTX 5080 with 16303 MiB** — not 96 GB. WP-28 measured this figure and WP-29 filed it as an erratum; it went unapplied. The AD-02 Draft-3 Intel→CUDA compose rewrite is still mandatory — the card IS NVIDIA. **What does NOT survive the correction:** the on-demand fp8-70B LLM-failover leg and the second-CUDA-video-node role (CogVideoX 5B / Wan2.1) were both sized against 96 GB and **do not fit 16 GB**. That is a design consequence, not a documentation fix, and is left OPEN for operator re-ruling — WP-53 D-1. node-02/03 trail on older worker tags. The `celery-worker-composition` on node-01 remains a bootstrap, not its permanent home.
